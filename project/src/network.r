...
df2 <- matrix_full[,colnames(matrix_full) %in% c("W_Ecosystm","Average_temp","elevation","Average_prec")]

aggregated_df2 <- aggregate(. ~ W_Ecosystm, data = df2 , FUN = mean)

df2_hclust <- aggregated_df2[,colnames(aggregated_df2) %in% c("Average_temp","elevation","Average_precip")]
